...
export class AgenceComponent implements AfterViewInit, OnInit {
  ...
  private geocodeAddress(address: string): Observable<any> {
    ...
    const headers = new HttpHeaders({
        'User-Agent': 'MyWebApplication/<email>'
    });
    ...
    return this.http.get<any[]>(url, { headers }).pipe(
        map((response: any[]) => {
            if (response && response.length > 0) {
                ...
                console.warn(`No geocoding result for address "${address}". Trying simplified query...`);
                // Try a simplified address if the first attempt fails
                const simplifiedAddress = address.split(',')[0]; // Take only the first part before a comma
                return this.trySimplifiedGeocode(simplifiedAddress, headers);
            }
        }),
        catchError(error => {
...
private trySimplifiedGeocode(address: string, headers: HttpHeaders): Observable<any> {
    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=json&addressdetails=1`;
    return this.http.get<any[]>(url, { headers }).pipe(
        map((response: any[]) => {
            if (response && response.length > 0) {
                console.log(`Simplified geocoding success for address "${address}":`, response[0]);
                return { lat: response[0].lat, lon: response[0].lon };
            } else {
                console.error(`Failed simplified geocoding for address "${address}"`);
                return undefined;
            }
        }),
        catchError(error => {
            console.error(`Error during simplified geocoding for address "${address}":`, error);
            return of(undefined);
        })
    );
}
